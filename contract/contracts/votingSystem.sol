...
contract votingSystem is voting {
    ...
    function vote(address candidate) external stillInDate {
    ...
    function actualCandidateState(address _candidate) public view  returns(uint, string, address){
        uint8 candidateId = getCandidateId(_candidate);
        Candidate memory candidate = Candidates[candidateId];
        return (candidate.votes, candidate.name, candidate.addr) ;
    }
    function getCandidateId(address _candidate) internal view  returns(uint8){
        require(_candidate != address(0),"invalid address");
        require(candidateToId[_candidate] >= 0, "candidate doesn´t exist");
